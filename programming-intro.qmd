...
Before you begin, make sure you’ve installed R and RStudio. We’ll also use a
GitHub-hosted package that includes code and templates for this course. This
means we will need to have Git installed to enable our computer to access the
code on the GitHub servers. Please navigate here to download the Git installer:
<https://git-scm.com>.
...
### A note on Quarto notebooks and rendering

We use Quarto notebooks as the medium for our problem sets. Quarto notebooks are
a way to explain and present quantitative analyses in one place. The *code
blocks* above

#### How do Quarto notebooks work?
...
The other way to run code in code blocks and see the results is by *rendering*
the
...
of the Quarto problem set worksheets.
...
agent interactions. We use the [`igraph`](https://r.igraph.org/) 
library to create social networks, or in some cases we use `socmod` code. 
We can create networks directly as follows, using the `igraph::make_graph` function.
...
### Data analysis and visualization in the *tidyverse*
