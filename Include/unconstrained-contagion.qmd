...
On each time step in Unconstrained Contagion, all the $N_A$ individuals doing $A$ will
collectively teach $\alpha N_A$ others from the population to switch their
behavior from the non-adaptive behavior $E$ to the adaptive behavior $A$.
This implies there will always be new individuals doing $E$ to switch to $A$,
i.e., the population is effectively infinite, and individuals are
indistinguishable, the large-$N$ approximation. Each member of the 
population is equally likely to interact with every other, i.e., the population
is *well-mixed*. This assumption will be relaxed to make room for the
constraints social networks impose on social transmission.

The change in the *prevalence* of an adaptation over one discrete time step is 
$$
\Delta a_t = \alpha a_t - \delta a_t
$$
